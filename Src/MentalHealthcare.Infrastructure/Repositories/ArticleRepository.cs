        ...
        ) : IArticleRepository
    {
        ...
        public async Task<(int, IEnumerable<Article>)> GetAllArticlesAsync(string? search, int requestPageNumber, int requestPageSize, string? sortBy)
        {
            ...
            query = sortBy?.ToLower() switch
            {
                ...
                _ => query.OrderBy(a => a.ArticleId)
            };
            //TODO : Pagination validation
            requestPageNumber = Math.Max(1, requestPageNumber);
            requestPageSize = Math.Max(1, requestPageSize);
            search ??= string.Empty;
        ...
        public async Task UpdateArticleAsync(Article articles)
        {
            ...
            var existingArticle = await dbContext.Articles.FindAsync(articles.ArticleId);
            if (existingArticle == null)
            {
                throw new ResourceNotFound(nameof(Article), articles.ArticleId.ToString());
            }
            dbContext.Entry(existingArticle).CurrentValues.SetValues(articles);
            await dbContext.SaveChangesAsync();
