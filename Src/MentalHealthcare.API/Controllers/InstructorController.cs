...
{using MentalHealthcare.Application.Advertisement.Commands.Create;
  ...
  //  [Route("api/[controller]")]
   // [ApiController]
    public class InstructorController ( IMediator mediator) : ControllerBase
    {
        
        [HttpPost]
        [SwaggerOperation(Summary = "Creates new Instructor", 
        ...
        [SwaggerOperation(Summary = "Delete The Instructor")]

        [HttpDelete("{instructorId}")]
        public async Task<IActionResult> DeleteInstructorId([FromRoute] int instructorId)
        {
            var command = new DeleteInstructorCommand()
            {
                InstructorID = instructorId
            };
