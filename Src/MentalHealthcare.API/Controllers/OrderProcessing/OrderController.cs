...
) : ControllerBase
{
    ...
    public async Task<IActionResult> PlaceOrder([FromBody] PlaceOrderCommand command)
    {
        ...
        var op = OperationResult<object>.SuccessResult(new
        {
            invoiceid = res,
        });
        return Ok(op);
    ...
    public async Task<IActionResult> GetAllOrders([FromQuery] GetAllInvoicesQuery query)
    {
        ...
        var op = OperationResult<PageResult<InvoiceViewDto>>
            .SuccessResult(res);
        return Ok(op);
    }

    [HttpGet("{invoiceId}")]
    [SwaggerOperation(Summary = "Retrieve a specific invoice.",
        Description = OrderProcessingDocs.GetInvoiceDescription)]
    [ApiExplorerSettings(GroupName = Global.SharedVersion)]
    public async Task<IActionResult> GetInvoice([FromRoute] int invoiceId)
    {
        ...
        var res = await mediator.Send(query);
        var op = OperationResult<InvoiceDto>
            .SuccessResult(res);
        return Ok(op);
    }

    [HttpPost("calculate-value")]
    [SwaggerOperation(Summary = "Calculate invoice value.",
        Description = OrderProcessingDocs.CalculateInvoiceDescription)]
    ...
    {
        var res = await mediator.Send(command);
        var op = OperationResult<CalculateInvoiceResponse>
            .SuccessResult(res);
        return Ok(op);
    }
    ...
    [SwaggerOperation(Summary = "Change invoice state.",
        Description = OrderProcessingDocs.ChangeInvoiceStateDescription)]
    [ApiExplorerSettings(GroupName = Global.DashboardVersion)]
    public async Task<IActionResult> ChangeInvoiceStatus([FromRoute] int invoiceId,
        [FromBody] ChangeInvoiceStateCommand command)
    {
        command.InvoiceId = invoiceId;
        await mediator.Send(command);
        return NoContent();
