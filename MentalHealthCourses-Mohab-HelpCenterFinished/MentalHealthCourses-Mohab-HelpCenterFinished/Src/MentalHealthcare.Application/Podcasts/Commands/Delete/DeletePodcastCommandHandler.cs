 ...
 IPodcastRepository PodRepo,
        ...
        ) : IRequestHandler<DeletePodcastCommand>
    {
        public async Task Handle(DeletePodcastCommand request, CancellationToken cancellationToken)
        {
            ...
            var Podcast = await PodRepo.GetByIdOrTitle(request.podcastId , request.title);
            var bunny = new BunnyClient(configuration);
            foreach (var PodCast in Podcast.PodcastDescription)
            {
                var PodcastName = GetPodcastName(Podcast.PodcastDescription);
                await bunny.DeleteFile(PodcastName, Global.PodCasterFolderName);
            }
