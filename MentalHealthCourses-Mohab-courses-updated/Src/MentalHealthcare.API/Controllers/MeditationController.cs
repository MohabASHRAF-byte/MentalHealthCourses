...
using MentalHealthcare.Application.Meditations.Command.Update;
...
namespace MentalHealthcare.API.Controllers
{
    ...
    public class MeditationController : ControllerBase
    {
        ...
        [HttpPost]
        [SwaggerOperation(Summary = "Create new Meditation", Description = "Creates new Article with its details")]
        ...
        [HttpPut]
        [SwaggerOperation(Summary = "Update Any Detail Of Meditation")]
        [Produces("application/json")]
        [ProducesResponseType(StatusCodes.Status204NoContent)]
        [ProducesResponseType(StatusCodes.Status400BadRequest)]
        public async Task<IActionResult> UpdateMeditation(UpdateMeditationCommand command)
        {
            await _mediator.Send(command);
            return NoContent();
        }
